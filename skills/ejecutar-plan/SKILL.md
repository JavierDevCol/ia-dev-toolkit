...
2. **Cargar plan:** leer `{hu_folder}/[ID-HU]/Plan.md`. Si Estado = `PENDIENTE` → cambiar HU a [E] En Ejecución y Plan.md a `EN_PROGRESO`, crear Tracking.md. Si Estado = `EN_PROGRESO` → **reanudar** (no reiniciar): leer `Tracking.md` (`Sección actual`) para ubicar el punto de continuación; no re-ejecutar tasks ya `[EJECUTADA]` ni repetir sus commits. Si Estado = `COMPLETADO` o `BLOQUEADO` → detener, no hay nada que ejecutar.
...
4. **Preparar entorno:** cargar reglas, preguntar rama base (main/develop/otra), crear rama con la convención de `git-branch-commit` (`hu-[ID-HU]-[desc-kebab]`), y `git stash` si hay cambios sin commit. Detectar framework de tests.
5. **Ejecutar:** por cada fase/task, EDITAR Plan.md (`[PENDIENTE]`→`[EN_PROGRESO]`→`[EJECUTADA]` y `- [ ]`→`- [X]`) en cada transición. Aplicar reglas arquitectónicas. Commit por task: Modo Plano → `feat([ID-HU]-EJEC-NN): …`; Modo Particionada → `feat([ID-HU]-TASK-N): …`. Máximo 2 reintentos por tarea; fallo → DETENER.
6. **Validar CAs:** delegar en un sub-agente que ejecute la skill `validar-ca` (`>validar_ca`). Veredicto real por CA: `CUMPLIDO` / `PARCIAL` / `NO CUMPLIDO` (nunca "FAIL"). Cualquier CA `PARCIAL` o `NO CUMPLIDO` → DETENER, no completar ni commitear el cierre de la HU; reportar la observación al usuario. Solo con todos los CA `CUMPLIDO` se avanza al paso 7.
...
| Plan no está PENDIENTE ni EN_PROGRESO | Ya completado o bloqueado | Verificar flujo de estados |
