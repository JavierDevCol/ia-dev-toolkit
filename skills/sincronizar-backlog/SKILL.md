...
digraph sincronizar_backlog {
  ...
  plan_exists [label="Refinamiento:\n¿Aprobación /\nBloqueo / Plan.md?", shape=diamond, style=filled, fillcolor="#F5A623"];
  ...
  start -> scan;
  scan -> plan_exists;
  plan_exists -> pending [label="No existe\nRefinamiento"];
  plan_exists -> refined [label="Sin Aprobación,\nsin Bloqueo"];
  plan_exists -> blocked [label="Con\n## Bloqueo de\nValidación"];
  plan_exists -> approved [label="Con\n✅ Aprobada"];
  plan_exists -> plan_status [label="Aprobada,\ncon Plan.md"];
...
| `[R] Refinada` | Refinamiento SIN `## Aprobación` y SIN `## Bloqueo de Validación` |
...
| `[B] Bloqueada` | Refinamiento CON `## Bloqueo de Validación` (sin `## Aprobación`) **O** Plan.md con Estado = `BLOQUEADO` |

**`[B]` no requiere `Plan.md`.** Una HU puede bloquearse en `>validar_hu` antes de llegar a planificación — el Refinamiento queda con `## Bloqueo de Validación` y nunca se genera `Plan.md`. Comprobar esa sección ANTES de asumir que "sin Plan.md" implica `[R]`.
